...
@{N='04 Segurança';C={ $x=H '04. SEGURANÇA';Get-NetFirewallProfile|%{$x+="`r`nFirewall $($_.Name): $($_.Enabled)"};$t=Get-Tpm;$x+="`r`nTPM: Presente=$($t.TpmPresent) Pronto=$($t.TpmReady) Versão=$($t.ManufacturerVersion)";$b=Get-BitLockerVolume;$b|%{$x+="`r`nBitLocker: $($_.MountPoint) | $($_.VolumeStatus) | $($_.ProtectionStatus)"};$x+="`r`nSecure Boot: $(if(Confirm-SecureBootUEFI){'Ativado'}else{'Não confirmado'})";$x }}
@{N='05 Updates';C={ $x=H '05. WINDOWS UPDATE / PATCHES';Get-HotFix|sort InstalledOn -Desc|select -First 30|%{$x+="`r`n  - $($_.HotFixID) | $($_.InstalledOn) | $($_.Description)"};$x }}
@{N='06 Software';C={ $x=H '06. SOFTWARE / DRIVERS';Get-ItemProperty 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*','HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*','HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*'|?DisplayName|sort DisplayName|select -First 150|%{$x+="`r`n  - $($_.DisplayName) v$($_.DisplayVersion) | $($_.Publisher)"};$x }}
@{N='07 Startup';C={ $x=H '07. SERVIÇOS / STARTUP';Get-Service|?Status -eq Running|select -First 150|%{$x+="`r`n  - $($_.Name) | $($_.DisplayName)"};foreach($p in 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Run','HKCU:\Software\Microsoft\Windows\CurrentVersion\Run'){$z=Get-ItemProperty $p;if($z){$z.PSObject.Properties|?Name -notmatch '^PS'|%{$x+="`r`nRUN: $($_.Name) = $($_.Value)"}}};$x }}
@{N='08 Usuários';C={ $x=H '08. USUÁRIOS / COMPARTILHAMENTOS';Get-LocalUser|%{$x+="`r`n  - $($_.Name) | Ativo=$($_.Enabled) | ÚltimoLogin=$($_.LastLogon)"};$x+='`r`nADMINISTRADORES:';Get-LocalGroupMember Administrators|%{$x+="`r`n  - $($_.Name)"};Get-SmbShare|%{$x+="`r`nSMB: $($_.Name) | $($_.Path)"};$x }}
@{N='09 Diagnóstico';C={ $x=H '09. DIAGNÓSTICO';Get-Process|sort WorkingSet64 -Desc|select -First 40|%{$x+="`r`n  - $($_.ProcessName) | PID=$($_.Id) | $([math]::Round($_.WorkingSet64/1MB,1)) MB"};$x+='`r`nPORTAS:';netstat -ano 2>$null|sls LISTENING|select -First 80|%{$x+="`r`n  - $($_.Line.Trim())"};$x }}
@{N='10 Recentes';C={ $x=H '10. ARQUIVOS RECENTES';$p=Join-Path $env:APPDATA 'Microsoft\Windows\Recent';Get-ChildItem $p -File|sort LastWriteTime -Desc|select -First $RecentFiles|%{$x+="`r`n  - $($_.LastWriteTime) | $($_.Name) | $($_.FullName)"};$x }}
@{N='11 Ambiente';C={ $x=H '11. AMBIENTE';'PATH','APPDATA','PROGRAMDATA','SYSTEMDRIVE','SYSTEMROOT','TEMP','TMP','USERNAME','COMPUTERNAME'|%{$v=[Environment]::GetEnvironmentVariable($_);$x+="`r`n$_ = $v"};$x }}
@{N='12 Downloads';C={ $x=H '12. DOWNLOADS';$f=Get-ChildItem "$env:USERPROFILE\Downloads" -File -Force -ErrorAction SilentlyContinue|sort LastWriteTime -Desc|select -First $DownloadItems;$x+="`r`nTotal coletado: $($f.Count)";$f|%{$x+="`r`n  - $($_.LastWriteTime) | $([math]::Round($_.Length/1KB,1)) KB | $($_.Extension) | $($_.FullName)"};$x+='`r`nRESUMO POR EXTENSÃO:';$f|group Extension|sort Count -Desc|%{$x+="`r`n  - $($_.Name): $($_.Count)"};$x }}
@{N='13 Navegadores';C={
